screen musicroom():
    ...
    default return_loop = []
    default music_channel = get_playing_channels()
    if mr.get_track(music_channel) not in mr.playlist:
        on "show" action [SetLocalVariable("return_track", mr.get_track(music_channel)), 
            SetLocalVariable("return_loop", renpy.music.get_loop(music_channel)), 
            Stop(music_channel),
            mr.Play()]
        on "replace" action [SetLocalVariable("return_track", mr.get_track(music_channel)), 
            SetLocalVariable("return_loop", renpy.music.get_loop(music_channel)), 
            Stop(music_channel),
            mr.Play()]

    if not main_menu:
        on "show" action [SetLocalVariable("current_track", mr.get_track(music_channel)), 
            SetLocalVariable("current_loop", renpy.music.get_loop(music_channel)),
            Stop(music_channel),
            mr.Play()]
        on "replace" action [SetLocalVariable("current_track", mr.get_track(music_channel)), 
            SetLocalVariable("current_loop", renpy.music.get_loop(music_channel)),
            Stop(music_channel),
            mr.Play()]
    if return_track:
        if not mr.get_track("music") == current_track:
            on "hide" action [Play(music_channel, return_loop if return_loop else return_track, loop=True if return_loop else False), 
                mr.Stop(), 
                If(not main_menu,Return())]
            on "replaced" action [Play(music_channel, return_loop if return_loop else return_track, loop=True if return_loop else False), 
                mr.Stop(), 
                If(not main_menu,Return())]
            key ("K_m", "game_menu") action [Play(music_channel, return_loop if return_loop else return_track, loop=True if return_loop else False), 
                mr.Stop(), 
                If(not main_menu,Return())]
        else:
            on "hide" action [Queue(music_channel, return_loop if return_loop else return_track, loop=True if return_loop else False), 
                mr.Stop(), 
                If(not main_menu,Return())]
            on "replaced" action [Queue(music_channel, return_loop if return_loop else return_track, loop=True if return_loop else False), 
                mr.Stop(), 
                If(not main_menu,Return())]
            key ("K_m", "game_menu") action [Queue(music_channel, return_loop if return_loop else return_track, loop=True if return_loop else False), 
                mr.Stop(), 
                If(not main_menu,Return())]
    else:
        key ("K_m", "game_menu") action mr.Stop(), If(not main_menu,Return())

    if current_track:
        if not mr.get_track("music") == current_track:
            on "hide" action [Play(music_channel, current_loop if current_loop else current_track, loop=True if current_loop else False), 
                mr.Stop(), 
                If(not main_menu,Return())]
            on "replaced" action [Play(music_channel, current_loop if current_loop else current_track, loop=True if current_loop else False), 
                mr.Stop(), 
            If(not main_menu,Return())]
            key ("K_m", "game_menu") action [Play(music_channel, current_loop if current_loop else current_track, loop=True if current_loop else False), 
                mr.Stop(), 
                If(not main_menu,Return())]
        else:
            on "hide" action [Queue(music_channel, current_loop if current_loop else current_track, loop=True if current_loop else False), 
                mr.Stop(), 
                If(not main_menu,Return())]
            on "replaced" action [Queue(music_channel, current_loop if current_loop else current_track, loop=True if current_loop else False), 
                mr.Stop(), 
                If(not main_menu,Return())]
            key ("K_m", "game_menu") action [Queue(music_channel, current_loop if current_loop else current_track, loop=True if current_loop else False), 
                mr.Stop(), 
                If(not main_menu,Return())]
    else:
        key ("K_m", "game_menu") action mr.Stop(), If(not main_menu,Return())
    ...
    if not renpy.music.is_playing(music_channel) and persistent._start_music_on_enter:
        on "replace" action mr.Play()

    if not custom_keep_music_playing:
        # Restore the main menu music upon leaving.
        if config.main_menu_music:
            if not mr.get_track(music_channel) == config.main_menu_music:
        ...
        vbox:
            hbox:
                ...
                hbox:
                    ...
                    bar value (barvalue if not timer_active and renpy.music.is_playing('music') else barvalue_active):
                        if not timer_active:
...
screen music_overlay():
    timer .5 action GetMusicPlaying(get_playing_channels()) repeat True
    default shown = False
    ...
    if shown and mr.get_track(get_playing_channels()) and persistent._music_overlay:
        frame:
            align (1.0,0.12)
            padding (20,10,20,10)
            vbox:
                ...
                text "Looping {}".format(mr.get_track(get_playing_channels()) in renpy.music.get_loop(get_playing_channels())) size gui.bar_size-5
                hbox:
                    spacing 30
